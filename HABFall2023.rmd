...
### Coefficients of Correlation Non - Aggregatted

```{r}

df_CR <- na.omit(df_CR)
describe(df_CR)  #this will flag those variables that are categorical with an asterix
numericag <- char2numeric(df_CR)  #this makes all numeric
...
```{r}
#Take away NA values 
df_CR <- na.omit(ag_df_CR) 
pacf(df_CR$chl, lag.max = 7) #test
...
pacf2021 <- df_CR %>% 
  ...
  pull(chl) %>%
  pacf(lag.max=10)
plot(pacf2021, main = "Chlorophyll Non-Aggregatted PACF - 10 Days Hourly")
...
pacf2021 <- df_CR %>% 
  ...
  pull(phy) %>%
  pacf(lag.max=10)
plot(pacf2021, main = "Phycocanin Non-Aggregatted PACF - 10 Days Hourly")
...
pacf2021 <- df_CR %>% 
  ...
  pull(chl) %>%
  pacf(lag.max=10)
plot(pacf2021, main = "Chlorophyll Non-Aggregatted PACF - 10 Days Daily")
...
pacf2021 <- df_CR %>% 
  ...
  pull(phy) %>%
  pacf(lag.max=10)
plot(pacf2021, main = "Phycocanin Non-Aggregatted PACF - 10 Days Daily")
...
length(ts_chl_2021)
acf(ts_chl_2021,lag.max = 24*30, main = "Cholorophyll Hourly Aggregated ACF")
abline(h = c(0.3,0.6) , col = c("red","red")) #change to lagmax = 30 if want days 
#determine lag -talk to deyle?
acf(ts_phy_2021,lag.max = 24*30, main = "Phycocanin Hourly Aggregated ACF")
abline(h = c(0.3,0.6) , col = c("red","red"))

acf(ts_chl_2021,lag.max = 30, main = "Cholorophyll Daily Aggregated ACF")
abline(h = c(0.3,0.6) , col = c("red","red")) #change to lagmax = 30 if want days 
#determine lag -talk to deyle?
acf(ts_phy_2021,lag.max = 30, main = "Phycocanin Daily Aggregated ACF")
abline(h = c(0.3,0.6) , col = c("red","red"))
...
pacf2021 <- ag_df_CR %>% 
  ...
  pull(chl) %>%
  pacf(lag.max=7)
plot(pacf2021, main = "Chlorophyll Aggregatted PACF - 1 Week")
...
pacf2021 <- ag_df_CR %>% 
  ...
  pull(phy) %>%
  pacf(lag.max=7)
plot(pacf2021, main = "Phycocanin Aggregatted PACF - 1 Week")


```


### Coefficients of Correlation Aggregatted
...
### Coefficients of Correlation 9-12
...
ts_delphy_2021 <- morndf_CR %>% 
  filter(year(date) == 2021) %>%
  group_by(date) %>%
  summarise(across(c("chl","del_phy"),mean),.groups = "keep") %>%
  pull(del_phy)


length(ts_chl_2021)
acf(ts_chl_2021,lag.max = 24*30, main = "Cholorophyll Hourly 9 AM - 12 PM ACF")
abline(h = c(0.3,0.6) , col = c("red","red")) #change to lagmax = 30 if want days 
#determine lag -talk to deyle?
acf(ts_phy_2021,lag.max = 24*30, main = "Phycocanin Hourly 9 AM - 12 PM ACF")
abline(h = c(0.3,0.6) , col = c("red","red"))

acf(ts_chl_2021,lag.max = 30, main = "Cholorophyll Daily 9 AM - 12 PM ACF")
abline(h = c(0.3,0.6) , col = c("red","red")) #change to lagmax = 30 if want days 
#determine lag -talk to deyle?
acf(ts_phy_2021,lag.max = 30, main = "Phycocanin Daily 9 AM - 12 PM ACF")
abline(h = c(0.3,0.6) , col = c("red","red"))

acf(ts_delphy_2021,lag.max = 30, main = "1st Difference Phycocanin Daily 9 AM - 12 PM ACF")
abline(h = c(0.3,0.6) , col = c("red","red"))
...
pacf2021 <- morndf_CR %>% 
  ...
  pull(chl) %>%
  pacf(lag.max=7)
plot(pacf2021, main = "Chlorophyll 9 AM : 12 PM PACF - 1 Week")
...
pacf2021 <- morndf_CR %>% 
  ...
  pull(phy) %>%
  pacf(lag.max=7)
plot(pacf2021, main = "Phycocanin 9 AM : 12 PM PACF 1st Difference - 1 Week")
...
pacf2021 <- morndf_CR %>% 
  ...
  pull(del_phy) %>%
  pacf(lag.max=7)
plot(pacf2021, main = "1st Difference PACF Phycocanin 9 AM : 12 PM - 1 Week")
...
#Grab Data
autoplot(rate) + ggtitle ("Autoplot - 9 AM : 12 PM ") + ylab("Chlorophyll (RFU)") + theme(plot.title = element_text(hjust = 0.5))
...
#Fit Dataset
rate <- ts(morndf_CR[,'del_phy'],start = c(2015,5), frequency = 365 )
...
```{r}
lib_ins <- c(1,1024) # data up through end of 2022; this is the training set
lib_oos <- c(1025,1171) # leave this alone until we're nearly done with the project
